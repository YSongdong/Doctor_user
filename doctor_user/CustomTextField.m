...
- (void) textFieldDidChange:(id) sender {
    ...
    if ([self.delegateCustom respondsToSelector:@selector(CustomTextField:textFieldHeight:)]) {
        [self.delegateCustom CustomTextField:self textFieldHeight:self.frame.size.height];
    }
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if ([self.delegateCustom respondsToSelector:@selector(textField:shouldChangeRange:String:)]) {
        return [self.delegateCustom textField:textField shouldChangeRange:range String:string];
    }
    return YES;
}
//点击清空按钮
